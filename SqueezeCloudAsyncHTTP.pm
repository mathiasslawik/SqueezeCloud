package Plugins::SqueezeCloud::SqueezeCloudAsyncHTTP;
...
sub onRedirect {
    ...
    $log->info( 'Server redirected to ' . $request->uri );
    ...
    $request->uri(
        URI->new_abs( $uri, $request->uri )
    );

    $log->info( 'Redirect rewritten to ' . $uri );
